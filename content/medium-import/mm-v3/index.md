...
The outward normal of the inner face is ![](eq/inline-005.png), so
...
At ![](eq/inline-006.png), the magnetic field is


![equation](eq/display-016.png)


The outward normal is ![](eq/inline-007.png), so
...
Because ![](eq/inline-008.png) is infinitesimal, we can write the first-order approximation
...
Here ![](eq/inline-009.png) actually comes from the multivariable function ![](eq/inline-010.png). When varying ![](eq/inline-011.png), ![](eq/inline-012.png) are held fixed, so a partial derivative should be used:
...
<h2>Magnetic flux through the two θ-direction faces</h2>

![Theta-direction side faces](fig-3-fixed.png)

*Figure 3. The θ-direction side faces and their outward normal directions.*

We repeat the same projection procedure for the two ![](eq/inline-013.png)-direction faces. Although the form is similar to that for the radial faces, writing it out explicitly makes each normal projection clear.

<h3>Inner face at θ</h3>

The two edges of this face are ![](eq/inline-014.png) and ![](eq/inline-015.png), so


![equation](eq/display-029.png)


The outward normal of this face is ![](eq/inline-016.png), therefore
...
<h3>Outer face at θ+dθ</h3>

The other face is at ![](eq/inline-017.png), with outward normal ![](eq/inline-018.png):
...
<h3>Net outward flux through the two θ faces</h3>
...
As before, hold the other variables fixed and consider only the variation in ![](eq/inline-019.png):
...
<h2>Magnetic flux through the two z-direction faces</h2>
...
<h3>Lower face at z</h3>

The two edges of the lower face are ![](eq/inline-020.png) and ![](eq/inline-021.png), so


![equation](eq/display-043.png)


The outward normal of the lower face is ![](eq/inline-022.png), therefore
...
<h3>Upper face at z+dz</h3>

The outward normal of the upper face is ![](eq/inline-023.png):
...
<h3>Net outward flux through the two z faces</h3>
...
Substitute both ![](eq/inline-024.png) and ![](eq/inline-025.png):


![equation](eq/display-063.png)


Cancel the common factor ![](eq/inline-026.png):
...
The magnetic mirror is invariant under rotation about the ![](eq/inline-027.png) axis, so physical quantities do not depend on ![](eq/inline-028.png). Hence,
...
These two conditions must be distinguished: axisymmetry gives a zero partial derivative with respect to ![](eq/inline-029.png), whereas ![](eq/inline-030.png) is an additional assumption about the magnetic-field structure.
...
<h2>Using ∇·B=0 to obtain Bᵣ near the axis</h2>
...
Near the axis, if ![](eq/inline-031.png) varies sufficiently slowly in space, then in the present lowest-order approximation ![](eq/inline-032.png) can be treated as approximately constant with respect to ![](eq/inline-033.png).

More precisely, what is required here is that **near the axis, at fixed ![](eq/inline-034.png), the variation of ![](eq/inline-035.png) with ![](eq/inline-036.png) can be neglected at the order being retained.** Under axisymmetry, near the axis we can write
...
At the lowest retained order, ![](eq/inline-037.png) can be taken outside the integral with respect to ![](eq/inline-038.png).

To avoid confusing the integration variable with the upper limit, write the ![](eq/inline-039.png) inside the integral as ![](eq/inline-040.png):
...
On the axis, ![](eq/inline-041.png); provided ![](eq/inline-042.png) remains finite, we have
...
If moving in the ![](eq/inline-043.png) direction, ![](eq/inline-044.png) slowly increases,
...
That is, the radial magnetic-field component points in the ![](eq/inline-045.png) direction.

<h2>Obtaining a force in the z direction from the radial magnetic field</h2>

The particle is also undergoing gyromotion; denote its azimuthal velocity by ![](eq/inline-046.png).
...
Under the present axisymmetric, ![](eq/inline-047.png) condition,
...
We consider only the ![](eq/inline-048.png) component of the Lorentz force. The term in the cross product that can produce a ![](eq/inline-049.png) component is
...
<h3>Relating r to the Larmor radius</h3>
...
To connect this directly to the ![](eq/inline-050.png) in the preceding equation, first consider a **special orbit whose guiding center lies exactly on the axis**. In this near-axis local model, the particle's gyro-orbit is centered on the axis, so the particle's distance from the axis is the Larmor radius,
...
When ![](eq/inline-051.png),


![equation](eq/display-100.png)


Because the magnitude of the gyromotion velocity is ![](eq/inline-052.png),
...
Substitute this back into ![](eq/inline-053.png):
...
<h3>Rewriting the z-direction result as a force along the magnetic field</h3>
...
First, near the axis the radial component ![](eq/inline-054.png) is small and the magnetic field is mainly in the ![](eq/inline-055.png) direction, so
...
At the same time, the ![](eq/inline-056.png) direction is approximately the magnetic-field-line direction, so
...
This is the standard magnetic mirror-force form. The explicit calculation above treats the near-axis special case in which the guiding center lies on the axis; for a general guiding-center position, the fast gyro-phase must be averaged. First-order guiding-center theory gives the same standard result ![](eq/inline-057.png).

<h3>What conditions of validity are implicit in this derivation?</h3>

Using local Larmor gyromotion and the magnetic moment ![](eq/inline-058.png) also assumes that the magnetic field varies slowly over one Larmor-radius scale; typically,


![equation](eq/display-117.png)


where ![](eq/inline-059.png) is the spatial scale over which the magnetic field changes significantly.
...
1. Start from ![](eq/inline-060.png);
...
4. The particle's gyromotion velocity ![](eq/inline-061.png) couples to the small radial magnetic field ![](eq/inline-062.png) through the Lorentz force and produces a force in the ![](eq/inline-063.png) direction;
